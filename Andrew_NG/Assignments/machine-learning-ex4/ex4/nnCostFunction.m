...
z3 = Theta2 * a2';
h_theta = sigmoid(z3);
%fprintf('h_theta computed');
%y(k) - we need to recode tthe labels as vectors containing values only 0
%or 1
y_new = zeros(num_labels, m); % 10*5000
for i=1:m
    %fprintf('Inside for loop in part1');
  y_new(y(i),i)=1;

end
%fprintf('outside for loop in part 1');
...
%fprintf('regularization completed');
J = J + reg;
